...
function addQuotes($strs){
	...
	$out = count($strs)>1 ? ($out . "'" .$strs[count($strs)-1]."'") : ($out."'" .$strs[count($strs)-1]."'");
		...
		function print_error($str = "")
		{
			...
			// Log to the server error log
			error_log("PowerSign: SQL error: $str");
			error_log("PowerSign: Query: $this->last_query");

			// If we want to abort on error, do it and bail
		...
		function query($query)
		{
			...
			// SET
			elseif ( preg_match("/^(set|alter|create|drop|start|commit|rollback)\\b/i",$query) )
			{
				$this->num_rows = 0;
				$return_val = 0;
			}
			...
			else
			{
				...
				while ($i < mysqli_num_fields($this->result))
				{
					...
					if ($this->col_info[$i]->name == '') {
						# we have an empty field name, this is not allowed, it breaks
						# mysql_fetch_object below
						$this->print_error("Empty field name!  Please use AS to name your field.");
						exit();
					}
					$i++;
				}
		...
		function debug()
		{
			...
			foreach ($traces as $trace) {
				if (substr($trace['file'], -15) != 'class.mysql.php') {
					break;
				}
			}
...
$mydb = new db("root", "12345", "cms", "localhost");
